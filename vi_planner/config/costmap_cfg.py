...
Loader.add_constructor(
    "tag:yaml.org,2002:python/object:viplanner.config.costmap_cfg.GeneralCostMapConfig",  # noqa
...
Loader.add_constructor(
    "tag:yaml.org,2002:python/object:viplanner.config.costmap_cfg.ReconstructionCfg",  # noqa
...
Loader.add_constructor(
    "tag:yaml.org,2002:python/object:viplanner.config.costmap_cfg.SemCostMapConfig",  # noqa
...
Loader.add_constructor(
    "tag:yaml.org,2002:python/object:viplanner.config.costmap_cfg.TsdfCostMapConfig",  # noqa
...
class ReconstructionCfg:
    ...
    # directory where the environment with the depth (and semantic)
    # images is located
    ...
    # (meaning that the depth images are also taked by the semantic camera)
    ...
    # maximum number of images to reconstruct, if None, all images are used
    ...
    # 3d points of nbr images added to point cloud at once
    # (higher values use more memory but faster)
...
class SemCostMapConfig:
    ...
    # ration of points that have to change by at least the #change_decimal
    # decimal value to converge
    conv_crit: float = 0.45
    # number of tasks for parallel processing, if None, all available
    # cores are used
    nb_tasks: Optional[int] = 10
    sigma_smooth: float = 2.5
    max_iterations: int = 1
    # obstacle threshold  (multiplied with highest loss value defined for
    # a semantic class)
    obstacle_threshold: float = 0.8  # 0.5/ 0.6 for matterport, 0.8 for carla
    # negative reward for space with smallest cost
    # (introduces a gradient in area with smallest loss value,
    # steering towards center)
    # NOTE: at the end cost map is elevated by that amount to
    # ensure that the smallest cost is 0
    negative_reward: float = 0.5
    # loss values rounded up to decimal #round_decimal_traversable equal to
    # 0.0 are selected and the traversable gradient is determined based on them
...
class GeneralCostMapConfig:
    ...
    # map parameters
    # cost map expansion over the point cloud space
    # (prevent paths to go out of the map)
    clear_dist: float = 1.0
    # smoothing parameters
    sigma_smooth: float = 3.0
    # cost map expansion
    x_min: Optional[float] = -8.05
    # [m] if None, the minimum of the point cloud is used None
    # (carla town01:  -8.05   matterport: None)
    y_min: Optional[float] = -8.05
    # [m] if None, the minimum of the point cloud is used None
    # (carla town01:  -8.05   matterport: None)
    x_max: Optional[float] = 346.22
    # [m] if None, the maximum of the point cloud is used None
    # (carla town01:  346.22  matterport: None)
    y_max: Optional[float] = 336.65
    # [m] if None, the maximum of the point cloud is used None
    # (carla town01:  336.65  matterport: None)
