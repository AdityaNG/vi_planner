...
class DataCfg:
    ...
    "all odom points with cost of more than obs_cost_height are discarded (negative cost of cost_map will be automatically added)"  # noqa
    fov_scale: float = 1.0
    ...
    "maximum number of train pairs (can be used to limit training time) can be set, otherwise number of recorded images times pairs_per_image is used"  # noqa
    ratio_fov_samples: float = 1.0
    ratio_front_samples: float = 0.0
    ratio_back_samples: float = 0.0
    "samples distribution -> either within the robots fov, in front of the robot but outside the fov or behind the robot"  # noqa

    # edge blur (real world RealSense difficulties along edges)
    # --> will be also visible in rgb/sem images due to warp
    ...
    # noise augmentation --> will be applied to a scaled image with range
    # between [0, 1]
    ...
    # Randomly set this proportion of images to complete black images  -->
...
class TrainCfg:
    ...
    "file path to models and data directory, can be overwritten by environment variable EXPERIMENT_DIRECTORY (e.g. for cluster)"  # noqa
    # NOTE: since the environment variable is intended for cluster usage,
    # some visualizations will be automatically switched off
    ...
    "if true, all samples will be loaded into RAM s.t. do not have to be reloaded for each epoch"  # noqa
    ...
    "obstacle threshold to decide if fear path or not (neg reward for semantic cost-maps is added automatically)"  # noqa
    ...
    "loading of a pre-trained rgb encoder from mask2former (possible is ResNet 50 or 101)"  # noqa
    # NOTE: `pre_train_cfg` and `pre_train_weights` are assumed to be found
    # under `file_path/models` (see above)
    ...
    def get_model_save(self, epoch: Optional[int] = None):
        ...
        return f"plannernet_env{self.env_list[0]}_ep{epoch}_input{input_domain}_cost{cost_name}_optim{optim}{hierarch}{name}"  # noqa
